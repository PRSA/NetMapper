...
public class NetworkGraph
{
        ...
        for(NetworkDevice device : deviceMap.values())
        {
            ...
            for(Map.Entry<Integer, String> entry : neighbors.entrySet())
            {
                ...
                for(NetworkDevice candidate : deviceMap.values())
                {
                    if(candidate == device)
                        continue;
                    // Strict match on sysName if available
                    if(candidate.getSysName() != null && neighborInfo.startsWith(candidate.getSysName()))
                    {
                ...
                if(targetDeviceId != null)
                {
                    ...
                    // simplifcation)
                    if(isValidPhysicalLink(srcIf, null))
                    {
                ...
                for(DetectedEndpoint endpoint : macEntry.getValue())
                {
                    ...
                    if(targetDeviceId != null)
                    {
                        ...
                        if(!sourceDeviceId.equals(targetDeviceId))
                        {
                            ...
                            if(isValidPhysicalLink(netInterface, targetIf))
                            {
    ...
    private static boolean isValidPhysicalLink(NetworkInterface src, NetworkInterface target)
    {
        if(src == null)
            return true; // Can't validate, assume ok? Or conservative?
