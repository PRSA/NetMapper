...
public class NetworkGraph {
            ...
            for (Map.Entry<Integer, List<DetectedEndpoint>> macEntry : device.getMacAddressTable().entrySet()) {
                ...
                for (DetectedEndpoint endpoint : macEntry.getValue()) {
                    ...
                    if (targetDeviceId != null) {
                        ...
                        if (!isInfrastructureDevice(device.getDeviceType())) {
                            epEdge.setType(EdgeType.LOGICAL);
                        } else if (ctx.isInfrastructurePort(device.getIpAddress(), interfaceIndex)) {
    ...
    private static int getCoreRank(NetworkDevice dev) {
        ...
        // Leaf devices: Hosts, Servers, MSA
        return 10;
    }
    ...
    private static boolean isInfrastructureDevice(String type) {
        ...
        return t.contains("switch") || t.contains("router") || t.contains("firewall")
                || t.contains("gateway") || t.contains("bridge") || t.contains("hub");
    }
        ...
        for (int i = 0; i < deviceEdges.size(); i++) {
            ...
            for (NetworkDevice devC : deviceMap.values()) {
                ...
                if (betterRank || betterLocal || tieBreak) {
                    ...
                    if (ipB.equals("10.81.128.1") || ipA.equals("10.81.128.1")) {
                        System.out.println("  -> PRUNED by " + ipC);
                    }
                    edge.setType(EdgeType.LOGICAL);
                    break;
                }
            }
            if ((ipB.equals("10.81.128.1") || ipA.equals("10.81.128.1")) && edge.getType() == EdgeType.PHYSICAL) {
                System.out.println("  -> KEPT link " + ipA + " <-> " + ipB);
            }
        }
                    ...
                    if (!commonPorts.isEmpty()) {
                        ...
                        if (!aPortsToCore.isEmpty() && !aPortsToB.isEmpty()) {
                            ...
                            if (!aCommon.isEmpty()) {
                                ...
                                if (ipB.equals("10.81.128.1")) {
                                    System.out.println("TRIANGLE CHECK: " + ipA + " -> " + ipB + " via " + coreIp);
                                    System.out.println(
                                            "  RankCore: " + rankCore + " RankA: " + rankA + " RankB: " + rankB);
                                    System.out.println("  DistCore: " + distCore + " vs DistA: " + distA
                                            + " -> Better: " + cIsBetter);
                                }
    ...
    private static boolean isPhysicalPort(NetworkDevice dev, Integer portIndex) {
        ...
        for (NetworkInterface ni : dev.getInterfaces()) {
            if (ni.getIndex() == portIndex) {
                ...
                if (desc.contains("lo0") || desc.contains("loopback") || desc.contains("null")
                        || desc.contains("tun") || desc.contains("tap")
                        || desc.contains("cpu") || desc.contains("stack")) {
                    return false;
                }
