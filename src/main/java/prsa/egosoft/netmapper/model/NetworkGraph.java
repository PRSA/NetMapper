...
public class NetworkGraph
{
    ...
    private static void applyPhysicalRedundancyFilter(NetworkGraph graph, Map<String, NetworkDevice> deviceMap,
            GraphContext ctx)
    {
        // NEW: First pass - identify and mark transitive links to non-switches
        // Heuristic: If a switch port has many MACs (>10), it's likely an uplink,
        // so any link to a non-switch through this port is transitive
        for(GraphEdge edge : graph.getEdges())
        {
            if(edge.getType() == EdgeType.LOGICAL)
                continue;
            if(!edge.getTargetId().startsWith("device_"))
                continue;
            ...
            // Only apply heuristic to links FROM switch TO non-switch
            String sourceType = sourceDev.getDeviceType();
            String targetType = targetDev.getDeviceType();
            boolean sourceIsSwitch = (sourceType != null && sourceType.equalsIgnoreCase("Switch"));
            boolean targetIsSwitch = (targetType != null && targetType.equalsIgnoreCase("Switch"));
            
            if(!sourceIsSwitch || targetIsSwitch)
                continue;
            
            // Check if the port seeing the target has many MACs (uplink indicator)
            java.util.Set<Integer> ports = ctx.getPortsViewingTarget(sourceIp, "device_" + targetIp);
            for(Integer portIdx : ports)
            {
                // Count total MACs on this port
                int macCount = 0;
                for(Map.Entry<Integer, List<DetectedEndpoint>> macEntry : sourceDev.getMacAddressTable().entrySet())
                {
                    if(macEntry.getKey().equals(portIdx))
                    {
                        macCount = macEntry.getValue().size();
                        break;
                    }
                }
                
                // Heuristic: If port has > 10 MACs, it's likely an uplink, not a direct
                // connection
                if(macCount > 10)
                {
        ...
        for(int i = 0; i < deviceEdges.size(); i++)
        {
            ...
            // NEW: For links to non-switches (hosts/servers), select the best link
            // Heuristic: Among all switches that see a host, only the one seeing it through
            // a port with MINIMUM number of MACs is the direct physical connection.
            // Ports with few MACs = access ports. Ports with many MACs = uplinks/transit.
            NetworkDevice devA = deviceMap.get(ipA);
            NetworkDevice devB = deviceMap.get(ipB);
            if(devA != null && devB != null)
            {
                String typeA = devA.getDeviceType();
                String typeB = devB.getDeviceType();
                boolean aIsSwitch = (typeA != null && typeA.equalsIgnoreCase("Switch"));
                boolean bIsSwitch = (typeB != null && typeB.equalsIgnoreCase("Switch"));
                
                // Only apply to links FROM switch TO non-switch
                if(aIsSwitch && !bIsSwitch)
                {
                    // Find all switches that see this host and their MAC counts
                    int minMacCount = Integer.MAX_VALUE;
                    String bestSwitchIp = null;
                    
                    for(NetworkDevice sw : deviceMap.values())
                    {
                        String swType = sw.getDeviceType();
                        if(swType == null || !swType.equalsIgnoreCase("Switch"))
                            continue;
                        
                        java.util.Set<Integer> portsToB = ctx.getPortsViewingTarget(sw.getIpAddress(), "device_" + ipB);
                        for(Integer portIdx : portsToB)
                        {
                            // Count MACs on this port
                            for(Map.Entry<Integer, List<DetectedEndpoint>> macEntry : sw.getMacAddressTable()
                                    .entrySet())
                            {
                                if(macEntry.getKey().equals(portIdx))
                                {
                                    int macCount = macEntry.getValue().size();
                                    if(macCount < minMacCount)
                                    {
                                        minMacCount = macCount;
                                        bestSwitchIp = sw.getIpAddress();
                                    }
                                    break;
                                }
                            }
                    ...
                    // If this switch is NOT the best one, mark link as LOGICAL
                    if(bestSwitchIp != null && !ipA.equals(bestSwitchIp))
                    {
                        edge.setType(EdgeType.LOGICAL);
                        continue;
                    }
    ...
    private static boolean isCoreBackbone(String ipA, String ipB, GraphEdge edge, Map<String, NetworkDevice> deviceMap)
    {
        ...
        if(devA != null && devB != null)
        {
            ...
            if((nameA.contains("core") || nameA.contains("dist")) && (nameB.contains("core") || nameB.contains("dist")))
            {
                return true;
            }
        }
    ...
    private static boolean isPhysicalPort(NetworkDevice dev, Integer portIndex)
    {
        ...
        for(NetworkInterface ni : dev.getInterfaces())
        {
            if(ni.getIndex() == portIndex)
            {
                ...
                if(type.startsWith("53 ") || type.startsWith("24 ") || type.startsWith("1 ") || type.startsWith("161"))
                {
