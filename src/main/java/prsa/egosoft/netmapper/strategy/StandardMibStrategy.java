...
/**
 * Estrategia estándar de descubrimiento usando MIB-II (RFC 1213). Soporta la
 * mayoría de routers y switches para información básica.
 */
public class StandardMibStrategy implements DiscoveryStrategy {
    ...
    public void discover(SnmpClient snmp, NetworkDevice device) {
        ...
        try {
            ...
            System.err.println("Error fetching MAC table: " + e.getMessage());
        }
        ...
        try {
            ...
            System.err.println("Error fetching VLANs: " + e.getMessage());
        }
     ...
     * wlp0s20f3.35) This is a fallback when Q-BRIDGE-MIB is not available.
     */
    private void detectVlansFromInterfaceNames(NetworkDevice device) {
        // Define multiple patterns for different OS conventions
        Pattern[] vlanPatterns = { Pattern.compile("^(.+)\\.(\\d{1,4})$"), // Linux/macOS: eth0.100
                ...
                if (matcher.find()) {
                    try {
                        ...
                        if (vlanId >= 1 && vlanId <= 4094) {
                            ...
                            String vlanEntry = "VLAN " + vlanId + " (" + prsa.egosoft.netmapper.i18n.Messages
                                    .getString("vlan.detected_from", detectionMethod, descr) + ")";
    ...
    private void detectLayer(NetworkDevice device) {
        ...
        } else if (type.contains("server") || type.contains("storage") || type.contains("msa")
                || descr.contains("esxi")) {
