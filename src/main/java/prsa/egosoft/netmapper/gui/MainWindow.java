...
public class MainWindow extends JFrame {
	...
	private void initComponents() {
		...
		// Content Split: Tabs on left, Details on right
		JSplitPane mainSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, tabbedPane, detailsPanel);
	...
	private void setupSelectionListeners() {
		// Selection from Tree
		treePanel.setSelectionListener(device -> {
			// Find or create a GraphNode to show details (if needed)
			// For simplicity, we can just show the device info in DetailsPanel
			detailsPanel.showNodeDetails(new NetworkGraph.GraphNode(device.getIpAddress(), device.getIpAddress(),
					device.getDeviceType(), NetworkGraph.NodeType.DEVICE), device);
		});

		// Selection from Map
		mapPanel.setSelectionListener((obj, device) -> {
			if (obj instanceof NetworkGraph.GraphNode) {
				detailsPanel.showNodeDetails((NetworkGraph.GraphNode) obj, device);
			} else if (obj instanceof NetworkGraph.GraphEdge) {
				detailsPanel.showEdgeDetails((NetworkGraph.GraphEdge) obj);
			}
		});
	}
